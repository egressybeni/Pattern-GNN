...
from utils.third_party import (
    ...
    get_mask,
    DirectedHeterophilousGraphDataset,
)
...
def get_val_folds(config, args, y, timestamps=None):
    ...
    if config.split_method == "random":
        ...
        y, timestamps = y.clone().detach(), timestamps.clone().detach()
...
def get_aml_data_benchmark_full(args, config):
    ...
    if jay_filename not in os.listdir(config.data_dir):
        ...
        if args.collapse_multi:
            ...
            logging.info(f"df_edges.shape before = {df_edges.shape}")
            sum_cols = ['Amount Received', 'Amount Sent']
            mean_cols = ['Timestamp']
            max_cols = ['Is Laundering']
            df_edges_modes = df_edges.groupby(['from_id', 'to_id']).agg(lambda x: pd.Series.mode(x)[0])
            df_edges_sums = df_edges.groupby(['from_id', 'to_id'])[sum_cols].sum()
            df_edges_means = df_edges.groupby(['from_id', 'to_id'])[mean_cols].mean()
            df_edges_maxs = df_edges.groupby(['from_id', 'to_id'])[max_cols].max()
            for col in sum_cols:
                df_edges_modes[col] = df_edges_sums[col]
            for col in mean_cols:
                df_edges_modes[col] = df_edges_means[col]
            for col in max_cols:
                df_edges_modes[col] = df_edges_maxs[col]
            df_edges = df_edges_modes.reset_index()
            logging.info(f"df_edges.shape after = {df_edges.shape}")
        ################# ####################### #################
...
def get_dir_dataset(name: str, root_dir: str, homophily=None, undirected=False, self_loops=False, transpose=False, seeds=1):
    path = f"{root_dir}/"
    evaluator = None
    ...
    if name in ["chameleon", "squirrel"]:
        ...
        for i in range(seeds):
            train_mask, val_mask, test_mask = get_dir_dataset_split(name, dataset.data, root_dir, i)
            tr_inds = train_mask + val_mask
            te_inds = test_mask
            val_folds.append((train_mask, val_mask))
    elif name in ["ogbn-arxiv"]:
        dataset = PygNodePropPredDataset(name=name, transform=T.ToSparseTensor(), root=path)
        ...
        dataset = DirectedHeterophilousGraphDataset(name=name, transform=T.NormalizeFeatures(), root=path)
        dataset.data.y = dataset.data.y.unsqueeze(-1)
        val_folds = []
        for i in range(seeds):
            train_mask, val_mask, test_mask = get_dir_dataset_split(name, dataset.data, root_dir, i)
    ...
    #     dataset = load_snap_patents_mat(n_classes=5, root=path)
        ...
        dataset = PygNodePropPredDataset(name="ogbn-arxiv", transform=T.ToSparseTensor(), root=path)
        ...
        # train_mask, val_mask, test_mask = get_dir_dataset_split(name, dataset.data, root_dir, 0)
        val_folds = []
        for i in range(seeds):
            train_mask, val_mask, test_mask = get_dir_dataset_split(name, dataset.data, root_dir, i)
            tr_inds = train_mask + val_mask
    ...
    # naive conversion to our GraphData class
    x = dataset.data.x
    y = dataset.data.y
    edge_index = dataset.data.edge_index
    edge_attr = torch.ones((edge_index.shape[1],1)).float()
    edge_timestamps = torch.ones((edge_index.shape[1],1)).float()
    tr_data = GraphData( x=x, y=y, edge_index=edge_index, edge_attr=edge_attr, timestamps=edge_timestamps)
    val_data = GraphData(x=x, y=y, edge_index=edge_index, edge_attr=edge_attr, timestamps=edge_timestamps)
    te_data = GraphData( x=x, y=y, edge_index=edge_index, edge_attr=edge_attr, timestamps=edge_timestamps)

    # tr_inds = train_mask + val_mask
    # te_inds = test_mask
    # val_folds = [(train_mask, val_mask)]
    return tr_data, val_data, te_data, val_folds, te_inds  


def get_dir_dataset_split(name, data, root_dir, split_number):
