...
class GraphSimulator(object):
    def __init__(
        ...
        elif generator == 'watts':
            self.generator = self.watts_strogatz_graph
    ...
    def watts_strogatz_graph(self, i=0, p=0.4):
        n = self.num_nodes
        avg_degree = self.avg_degree
        if i>0: avg_degree = np.random.poisson(avg_degree)
        k = round((avg_degree + 0.5))
        logging.info(f"Component {i} avg_degree = {k}")
        d = self.delta
        if p is None: p = (8*d-k) / (n-k)
        p = max(0, min(1,p))
        nx_graph = nx.watts_strogatz_graph(n, k, p, seed=0)
        nx_graph=nx.DiGraph([random.sample(e,2) for e in nx_graph.edges()])
